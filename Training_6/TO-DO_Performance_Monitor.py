"""
A simple program that gets data from the server in JSON format and converts  it into Python format. The data are taken form testing
{JSON} Placeholder service and contains, among others, list of users with tasks to do.
The program counts completed tasks by users and is chooseing the user with the largest number of completed tasks.
...
for user in users:
    if (user["id"] in usersWithTopCompletedTasks):
        print("Users with bigest amount of completed tasks is:", user["name"])
...
for userId in usersWithTopCompletedTasks:
    ...
    user = r.json()
    print("Users with bigest amount of completed tasks is:", user[0]["name"])
"""
